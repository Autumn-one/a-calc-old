...
**缓存方法应用得当的时候可以提供巨幅的性能提升！**

`calc` `fmt` `calc_wrap` 方法都有对应的缓存方法，方法名就是在对应的名称后面加上 `_memo`，用法与非缓存版本相同，只不过多了内置的缓存功能

```typescript
import { calc_memo, fmt_memo, calc_wrap_memo } from "a-calc"
```

这些缓存方法不适合用来计算完全随机的计算式，但是即便是完全随机的计算式也有办法改成缓存方法可利用版本，前提是不同的计算式之间的公式是可以复用的，有下面的示例：
...
可以看到，上方的计算式我们一旦提取出一种公式 `a + b` 那么就可以复用了，但是注意，`a + b` 与 `c + d` 被认为是不同的公式！

总之合理的利用缓存模式的计算可以大幅度提高性能，这是所有性能提升技巧中对性能提升最大的一个技巧。
