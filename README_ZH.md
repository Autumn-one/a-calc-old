...
默认情况下表达式中的空格不是必须的，除非你使用了space或者space-all模式，关于这两个模式的介绍在**高性能**章节有具体介绍，但是我推荐你始终在表达式中包含空格，这样看起来更清晰好看。
...
这个章节将会教你如何挤出a-calc最后一滴的性能，a-calc2.x在最高性能下的速度比1.x快了数倍之多。

### 记忆缓存

你可以使用 `_memo` 参数来开启计算缓存，多数情况我们会使用变量来参与计算，如果由变量组成的公式是相同的那么缓存也会被使用

```typescript
calc("a + b", {a：1， b: 2, _memo: true})
calc("c + d", {c: 8, d: 9, _memo: true})
// 上方两个计算式有一个性能的优化点没有做到就是公式不通用，同样是两数相加却使用了不同的公式，a + b 与 c + d 是不同的公式，虽然这是相同的模式，但是a-calc只会记忆相同的公式
...
默认情况下缓存功能是关闭的，这是因为缓存需要你自己决定在合适的时候进行清理，不然可能造成过量的内存占用，a-calc给你提供了一个清理函数，你只需在合适的时机调用它即可。

```typescript
import {clear_memo, get_memo_size} from "a-calc"

clear_memo() // 默认最大保留5000个缓存结果，但是如果缓存项被使用少于100次那么会被清除，当然你也可以自己设置
clear_memo({maximum: 10000, use_count: 200}) // maximum 表示清理之后最大保留到的缓存数目，use_count表示对于小于这个重用次数的缓存进行清理
get_memo_size() // return type: number 可以使用这个方法返回当前缓存的数量，你可以根据这个信息来决定要不要清理缓存
```

### space与space-all模式

space 与 space-all 模式都可以提升一定的性能，但是这两个模式对代码的编写提出了更高的要求，space模式要求在计算式部分的每个单元之间严格插入空格，space-all 不仅要求计算式部分严格插入空格并且要求fmt格式化部分也要插入空格
